  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (value && type === 'email' && !isValidEmail(value)) {
        showError(field, field.dataset.errorEmail || 'Please enter a valid email address.');
        return false;
      }

      if (value && type === 'tel' && !isValidPhone(value)) {
        showError(field, field.dataset.errorPhone || 'Please enter a valid phone number.');
        return false;
      }
      ...
      if (value && field.hasAttribute('minlength')) {
        ...
        if (value.length < min) {
          showError(field, field.dataset.errorMinlength || 'Input is too short.');
          return false;
        }
      }
      ...
      if (value && field.hasAttribute('maxlength')) {
        ...
        if (value.length > max) {
          showError(field, field.dataset.errorMaxlength || 'Input is too long.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isFormValid) {
        const firstError = contactForm.querySelector('.' + ERROR_CLASS);
        if (firstError) {
          firstError.focus();
      ...
      contactForm.classList.add(SUCCESS_CLASS);
      ...
      successMsg.textContent = contactForm.dataset.successMessage || 'Thank you! Your message has been sent.';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "AL משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'f048974c-4215-4ef6-a822-e9b1da144d7e',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'f048974c-4215-4ef6-a822-e9b1da144d7e';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'f048974c-4215-4ef6-a822-e9b1da144d7e';
